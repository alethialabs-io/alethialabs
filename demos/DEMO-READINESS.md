...
1. **Nothing new is hosted.** `main` (what the prod box serves) is ~#294. The whole 5-cloud
   ...
   to `main`.
2. **Nothing is proven on real infrastructure.** The `tofu apply → kubeconfig → Talos CNI →
   reachability probe → datapath probe → ArgoCD → addons` hero chain is CI-green + `tofu validate`-clean
   on 5 clouds but **has never run against a real cluster once**. CI does no real apply and no live
   plan. There is **no committed real-cluster proof artifact** anywhere — the `feat/e1-hetzner-proof`
   branch is stale (~#289) and holds only a template fix, not a proof.
...
| `RunDeployV2` apply → kubeconfig → CNI → reachability/datapath gates (#288/#301/#307/#308) | 🟡 CODE-ONLY | **never run live — the #1 demo blocker** |
| ArgoCD/addons `kubectl apply`, health, security read-back | 🟡 CODE-ONLY | |
...
|---|---|---|---|
| **0** | This ledger + Artifact dashboard + rollback record + budgets | auto + maintainer | 🟡 in progress |
| **1** | **E1** — first real Hetzner/Talos cluster, LOCAL (the #1 derisk) | maintainer (live infra) | 📋 |
