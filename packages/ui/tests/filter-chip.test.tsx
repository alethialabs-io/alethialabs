...
describe("FilterChip", () => {
  ...
  it("renders the count as tertiary ink when resting and inherited ink when filled", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Healthy 3" }),
    ).toBeInTheDocument();
  ...
  it("renders no count node when none is given", () => {
    ...
    // incidental: it would fail on any unrelated structural change (an icon slot, wrapping the
    // children) with a message about node counts rather than about a count node. What it did
    // catch that the accessible name cannot — an empty span, a stray `{" "}` — is the span
    // assertion below.
  ...
  // The two halves of one control brighten together: the count follows the label on hover
  // rather than staying at the dimmer tier while the label snaps to `--foreground`.
  it("gives the resting count a hover rung that matches the label", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Healthy 3" }).className,
    ).toContain("group");
  });
});
...
describe("FilterChipGroup", () => {
  ...
  it("passes an option's count through to its chip", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Healthy 12" }),
    ).toBeInTheDocument();
