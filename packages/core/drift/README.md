...
classifies them out on three rules, and each is deliberately narrow:
...
(`empty_collection` > `computed_attribute` > `undeclared_collection`), so the trail never overstates
