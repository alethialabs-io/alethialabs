...
// TestTofuCLI_LifecycleWrappers drives the init → validate → plan → show → apply → output →
// state-read → refresh-only → plan-destroy → destroy sequence against a real `tofu` binary,
// which is the only way these thin exec wrappers are exercised at all.
func TestTofuCLI_LifecycleWrappers(t *testing.T) {
	requireTofuOrSkip(t)
	...
	varFile, err := OverrideTfvarsFromMap(dir, map[string]interface{}{"note": "alpha"})
	if err != nil {
		t.Fatalf("OverrideTfvarsFromMap: %v", err)
	}

	if err := tf.InitWithBackendFile(ctx, backendFile, true); err != nil {
		t.Fatalf("InitWithBackendFile: %v\n%s", err, logBuf)
	}

	validated, err := tf.Validate(ctx)
	if err != nil {
		t.Fatalf("Validate: %v\n%s", err, logBuf)
	}
	if !validated.Valid {
		t.Fatalf("Validate reported invalid config: %+v", validated.Diagnostics)
	}

	planFile := filepath.Join(dir, "tofu.plan.out")
	hasChanges, err := tf.Plan(ctx, varFile, planFile)
	if err != nil {
		t.Fatalf("Plan: %v\n%s", err, logBuf)
	}
	if !hasChanges {
		t.Fatal("Plan reported no changes for an unapplied module")
	}

	planJSON, err := tf.ShowPlanJSON(ctx, planFile)
	if err != nil {
		t.Fatalf("ShowPlanJSON: %v\n%s", err, logBuf)
	}
	if planJSON == nil || len(planJSON.ResourceChanges) != 1 {
		t.Fatalf("ShowPlanJSON resource changes = %+v, want exactly 1", planJSON)
	}
	if got := planJSON.ResourceChanges[0].Address; got != "terraform_data.probe" {
	...
	if got, _ := outputs["kubeconfig"].(string); got != "kube-alpha" {
		t.Fatalf("kubeconfig output = %q, want kube-alpha", got)
	}
	...
	if len(addrs) != 1 || addrs[0] != "terraform_data.probe" {
		t.Fatalf("StateResources = %v, want [terraform_data.probe]", addrs)
	}

	// Refresh-only against an already-converged state: no drift, so no changes.
	drifted, err := tf.PlanRefreshOnly(ctx, varFile, filepath.Join(dir, "refresh.plan"))
	if err != nil {
		t.Fatalf("PlanRefreshOnly: %v\n%s", err, logBuf)
	}
	if drifted {
		t.Fatal("PlanRefreshOnly reported drift immediately after apply")
	}

	// A destroy PLAN must propose changes without performing them — state stays populated.
	destroyChanges, err := tf.PlanDestroy(ctx, varFile, filepath.Join(dir, "destroy.plan"))
	if err != nil {
		t.Fatalf("PlanDestroy: %v\n%s", err, logBuf)
	}
	if !destroyChanges {
		t.Fatal("PlanDestroy reported no changes for a populated state")
	}
	if addrs, err := tf.StateResources(ctx); err != nil || len(addrs) != 1 {
		t.Fatalf("PlanDestroy mutated state: addrs=%v err=%v", addrs, err)
	}

	if err := tf.Destroy(ctx, varFile); err != nil {
		t.Fatalf("Destroy: %v\n%s", err, logBuf)
	}
	// An emptied state has no root module values at all — the nil-state branch.
	if addrs, err := tf.StateResources(ctx); err != nil || len(addrs) != 0 {
		t.Fatalf("StateResources after Destroy = %v err=%v, want empty", addrs, err)
	}
}
