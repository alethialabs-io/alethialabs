...
func loadTestToken(t *testing.T) string {
	...
	credsPath := filepath.Join(configDir, "vtx", "credentials.json")
	data, err := os.ReadFile(credsPath)
	if err != nil {
		t.Skip("not logged in — run `vtx login` first")
	}
	...
	if err != nil {
		t.Skipf("token refresh failed: %v — run `vtx login` to re-authenticate", err)
	}
	return token
}

func refreshIfNeeded(creds types.ExchangeResponse, credsPath string) (string, error) {
	webOrigin := os.Getenv("VTX_WEB_ORIGIN")
	if webOrigin == "" {
...
func skipOnAuthOrNotFound(t *testing.T, err error, endpoint string) {
	...
	if strings.Contains(msg, "Unauthorized") || strings.Contains(msg, "Invalid token") {
		t.Skipf("auth failed for %s — run `vtx login` to refresh", endpoint)
	}
