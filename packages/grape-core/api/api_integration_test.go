...
func loadTestToken(t *testing.T) string {
	...
	if err != nil {
		t.Skip("not logged in — run `grape login` first")
	}
	...
	if err != nil {
		t.Skipf("token refresh failed: %v — run `grape login` to re-authenticate", err)
	}
	return token
}
...
func skipOnAuthOrNotFound(t *testing.T, err error, endpoint string) {
	...
	if strings.Contains(msg, "Unauthorized") || strings.Contains(msg, "Invalid token") {
		t.Skipf("auth failed for %s — run `grape login` to refresh", endpoint)
	}
