...
| **#1268** | Apply the account-B stack, set `E2E_SECRETS_XACCT*`, dispatch aws from `main`, run `secrets-e2e.sh aws strict`, then flip the `*-xacct` catalog rows. The GCP account-B stack is written: `infra/gcp-secrets-e2e` (#2311). Apply it after you create the standing external-secrets GSA in project A. The gcp lane adopts that GSA since #4856. Azure has no e2e account-B stack. Its leg needs a second Azure subscription in the same tenant. |
...
| **#3078** | Where azure's `cache` kind provisions. The failure itself is now caught for free — `t2RequireAzureManagedRedisPreflight` refuses `westeurope`/`Balanced_B0` before any spend instead of at ~1724s mid-apply — but a refusal is not a fix, and the run still cannot prove the cell. The three resolutions are mutually exclusive and each spends differently: **move the row's default region** (`.github/workflows/e2e-nightly.yml`'s `azure) DEFAULT_REGION` and `test/e2e/t2_providers.go`'s `defaultRegion`; swedencentral is where run 33115079834 completed), **pin a different Managed Redis family** (which changes what the cell claims — there is nothing below `Balanced_B0` in Balanced_*), or **re-test westeurope** with `ALETHIA_E2E_AZURE_REDIS_ALLOW_KNOWN_BAD=1`, which is a full paid apply on the bet that capacity came back. Deliberately not decided in the preflight PR: it changes where and what every azure run provisions. |
