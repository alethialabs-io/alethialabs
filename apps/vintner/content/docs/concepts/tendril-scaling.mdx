...
Tendrils run on AWS ECS Fargate with **scale-to-zero** enabled by default. A Lambda function polls the job queue every minute and spins up Tendril containers on demand. When no jobs remain, the same function scales them back down after a short idle period.
...
| Component | Role |
|-----------|------|
| **EventBridge rule** | Triggers the Lambda scaler every 1 minute |
...
### Lambda Detects Queued Jobs

The EventBridge rule fires (up to 60 seconds after the job was queued). The Lambda function queries Supabase's REST API for the count of `QUEUED` jobs using the `content-range` header.
...
<Callout type="info">
**Cold start latency**: worst case is ~2 minutes from job creation to job claimed — up to 60 seconds waiting for the Lambda poll, plus 30–60 seconds for ECS task startup. Subsequent jobs while the Tendril is already running are claimed within seconds.
</Callout>
