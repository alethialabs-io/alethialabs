...
Azure integration uses **Federated Identity Credentials** in Azure Entra ID (formerly Azure AD). The Tendril presents an OIDC token, Azure validates it, and issues a short-lived access token. No client secrets are generated. See [Security Architecture](/concepts/security).
