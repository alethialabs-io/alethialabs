...
export const metadata: Metadata = {
	...
	description: "How Alethia collects, uses, shares, and protects personal data.",
};

/** Public GDPR privacy notice for Alethia's website and hosted service. */
export default function PrivacyPage() {
	return (
		<LegalShell title="Privacy Policy" lastUpdated="July 29, 2026">
			<p>
				...
				processes infrastructure data solely on a business customer’s instructions,
				that customer is the controller and we are its processor under our{" "}
				<Link href="/legal/dpa">Data Processing Addendum</Link>.
			</p>
			<p>
				Alethia Labs DPK has been submitted for registration but has no EIK yet.
				Until registration is complete, the controller and contracting operator is{" "}
				{CURRENT_LEGAL_OPERATOR}. We will update this notice before the registered
				company assumes those roles.
			</p>
			...
			<p>
				...
				organization membership, role, and account preferences. Social sign-in may
				come from GitHub, GitLab, Bitbucket, or Google. Email sign-in uses a
				one-time code.
			</p>
			...
			<p>
				...
				operational logs, support conversations, and the metadata needed to manage
				your environments. Customer-supplied content can contain personal data, so
				you should submit only what is needed.
			</p>
			<h3>Credentials and connected services</h3>
			<p>
				Git-provider OAuth tokens are encrypted at rest. Cloud connections retain
				only federation configuration such as role, tenant, project, audience, and
				issuer identifiers. <strong>Alethia does not store static cloud access keys.</strong>{" "}
				Cloud access uses short-lived, federated credentials. Payment-card details
				are collected by Stripe and are not stored by Alethia.
			</p>
			...
			<ul>
				<li>
					<strong>Contract:</strong> create accounts, authenticate users, provide
					the Service, execute requested deployments, provide support, and bill
					paid plans.
				</li>
				<li>
					<strong>Legitimate interests:</strong> secure the Service, prevent abuse,
					maintain reliability, investigate failures, and keep proportionate
			...
			<p>
				...
				Cloudflare processes edge-network and security data. Transactional email is
				delivered through Amazon SES. Other providers are listed on our{" "}
			...
			<p>
				...
				providers may process data outside the EEA. Where required, transfers rely
				on an adequacy decision, the European Commission’s Standard Contractual
				Clauses, and supplementary technical and organizational measures.
			</p>

			<h2>5. Retention</h2>
			<ul>
				<li>
					Account, organization, project, and billing records: while the account is
					active, then deleted or anonymized after closure unless law or a dispute
				...
				<li>
					Removed cloud inventory and capability observations: 7 days by default.
				</li>
				<li>
					Support and contractual correspondence: up to 3 years after the matter
					closes; invoices and tax records: the period required by Bulgarian law.
				</li>
				<li>
					Optional telemetry: according to the provider retention configured by
					Alethia, never longer than 12 months for product analytics or 30 days for
					session replay.
				</li>
			</ul>
			<p>
				Self-hosted operators choose their own retention periods and are responsible
				for their own privacy notices. Deletion from encrypted backups may occur on
				the backup rotation rather than immediately; restored data is re-subjected
			...
			<p>
				...
				restriction, portability, or objection; withdraw consent; and complain to a
				supervisory authority. We may need to verify identity and may retain data
				where an exemption or legal duty applies. Start a request at{" "}
			...
			<p>
				...
				solely by automated means. The Service is for business and professional use
				and is not directed to children under 16.
			</p>

			<h2>9. Contact and changes</h2>
			<p>
				Privacy questions may be sent to{" "}
				<a href="mailto:privacy@alethialabs.io">privacy@alethialabs.io</a> or by post
				to {LEGAL_ENTITY.registeredAddress}. We have not appointed a Data Protection
				Officer because our current processing does not require one. We will update
				the date above and provide additional notice when a material change requires
				it.
			</p>
		</LegalShell>
	);
