...
/** 01 · Connect — attach a cloud via short-lived federated identity, store nothing. */
export function Connect() {
	return (
		<section style={{ padding: "84px 0", borderTop: "1px solid var(--border)" }}>
			<Wrap>
				<SecMark n="01" label="Connect a cloud" />
				...
				<div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }} className="ah-3col">
					{CONNS.map((c) => (
						<div key={c.id} style={{ border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", background: "var(--surface)", padding: 22 }}>
							<div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
								<Prov id={c.id} size={22} />
								<span style={{ ...disp, fontSize: 16, fontWeight: 600, color: "var(--text-primary)" }}>{c.name}</span>
