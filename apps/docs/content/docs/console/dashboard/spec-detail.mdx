---
title: Spec Detail
description: The spec detail page — services, infrastructure, plan, jobs, and lifecycle actions.
---
...
**Recent Jobs** — table showing the last 5 jobs for this spec (type, status, created, duration). Click to navigate to [job detail](/console/jobs/job-detail).
...
The **Duplicate & Edit** action opens the Design a Spec form pre-filled with the spec's current configuration. If you select a different cloud identity (e.g., switching from AWS to GCP), the configuration is converted automatically. See [Multi-Cloud Conversion](/concepts/multi-cloud-conversion) for how this works.
