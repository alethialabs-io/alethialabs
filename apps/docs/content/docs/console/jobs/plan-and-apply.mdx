...
2. Generates Terraform variable files from the project's config snapshot
3. Runs `terraform init` with S3-compatible backend ([state management](/concepts/terraform-state))
...
7. Uploads the plan artifact to S3-compatible storage
...
2. Validates the config hash — if the project was modified after planning, the apply fails with a hash mismatch error
...
5. Project status → DESTROYING → DESTROYED
