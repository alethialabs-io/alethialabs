...
Handles the OAuth redirect, handled by Better Auth at `/api/auth/[...all]` after provider authentication. Captures the authorization code and provider token, then redirects to the intended page.
