...
Compares the spec's current config hash against the plan's config hash. If they differ (spec was modified after planning), the job fails:
...
- Updates `spec_cluster` with cluster metadata and ArgoCD credentials
- Updates `spec_databases` with writer/reader endpoints and credential ARNs
- Updates `spec_caches` with primary/reader endpoints
- Sets spec status to `ACTIVE`

This data appears in the [Clusters page](/console/clusters) and spec detail.
