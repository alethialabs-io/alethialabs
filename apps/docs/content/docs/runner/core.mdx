...
RunDeployV2(projectConfig, provider, planFile, installerConfig, apiClient, dryRun)
  ...
  → Generate terraform.tfvars from ProjectConfig
      · brownfield: for a component marked "existing" (e.g. !ProvisionNetwork + NetworkID), inject the
        existing resource id as a tfvar (vpc_id / network_id / vnet_id) so the template attaches to it
        instead of creating one; AWS additionally resolves the VPC's subnets here (EC2 API)
...
Three implementations (provisioning-only — the runner no longer lists cloud resources; the console
maintains the [asset inventory](/console/connectors/health-and-inventory) server-side):
- `aws_provider.go` — AWS SDK: STS (AssumeRole), EKS (post-provision kubeconfig), S3 (state), Access
  Analyzer (verify gate), plus an EC2 subnet lookup for reusing an existing VPC.
