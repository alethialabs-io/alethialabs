...
After a Runner runs `terraform plan`, it optionally runs [Infracost](https://www.infracost.io/) for a precise cost breakdown based on the actual Terraform resources.
...
The Runner generates a plan JSON via `terraform show -json plan.tfplan`.
