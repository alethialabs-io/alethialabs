...
The platform provides two layers of cost estimation: a **real-time cost sidebar** during Spec configuration, and a **post-plan Infracost analysis** for precise per-resource costs.

<img src="/docs/diagrams/concepts/cost-estimation.svg" alt="Cost Estimation" />

## Cost Sidebar (Real-Time)

The cost sidebar appears on the right side of the Design a Spec form. As the user configures infrastructure, the estimated monthly cost updates immediately.
...
The Plan tab in the Spec detail page shows per-resource costs and a total monthly estimate.
