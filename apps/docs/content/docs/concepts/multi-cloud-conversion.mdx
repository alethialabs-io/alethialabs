...
description: How Spec configurations are converted across AWS, GCP, and Azure when duplicating.
---

# Multi-Cloud Conversion

Users can duplicate an existing Spec and target a different cloud provider. The platform converts the configuration automatically, mapping resources, instance types, and capacity units across providers.

<img src="/docs/diagrams/concepts/multi-cloud-conversion.svg" alt="Multi-Cloud Conversion" />

## When Conversion Happens

1. User clicks **"Duplicate & Edit"** on an existing Spec
2. Selects a different cloud identity (e.g., switching from AWS to GCP)
3. The `convertSpecConfig()` function transforms the configuration
...
- If the source Spec uses an **existing network** (`provision_network = false`), the conversion switches to **create new** (`provision_network = true`) with a warning: "Existing network cannot be reused across providers."
...
Warnings are displayed in the Design a Spec form after provider switch, giving the user a chance to review and adjust before submitting.
