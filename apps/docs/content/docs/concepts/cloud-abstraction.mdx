...
The same registry pattern extends one level down to **per-category connectors** — letting a Spec swap the cloud-native default for a tool you already run (Cloudflare DNS, Vault secrets, Docker Hub registry, Datadog/Grafana/Prometheus observability). See [Connectors Architecture](/concepts/connectors).
