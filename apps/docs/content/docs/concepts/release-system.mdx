...
description: release-please versioning, GoReleaser for Alethia, Docker builds for Tendril, and GitHub Actions workflows.
---

# Build, Release & CI/CD

Alethia CLI and Tendril (agent) are versioned and released independently using **release-please** with conventional commits. Each component has its own release workflow, tag format, and distribution channel.
...
1. Developers write conventional commits: `feat(alethia): add vine get command`, `fix(tendril): handle nil credential`
...
4. When merged, release-please creates a git tag (`alethia-v0.1.8` or `tendril-v0.1.0`)
