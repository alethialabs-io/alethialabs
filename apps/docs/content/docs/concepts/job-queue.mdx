...
| `PLAN` | Run `terraform plan` + optional Infracost analysis | Clicking "Plan" on a Project |
| `DEPLOY` | Run `terraform apply` + install ArgoCD | Clicking "Apply" on a Project |
| `DESTROY` | Run `terraform destroy` | Clicking "Destroy" on a Project |
| `DEPLOY_RUNNER` | Provision Runner infrastructure (ECS task, IAM roles) | Deploying a self-operated Runner |
| `UPDATE_RUNNER` | Update Runner to latest release | Clicking "Update" on a Runner |
| `DESTROY_RUNNER` | Tear down Runner infrastructure | Removing a deployed Runner |
...
### Job Created
User clicks "Plan" / "Apply" / "Destroy" in the UI (or runs `alethia plan` / `alethia apply` / `alethia destroy` in the CLI). A `jobs` row is inserted with status `QUEUED`.
...
### Finalization
For DEPLOY jobs, Console calls `finalizeDeployment()` to extract cluster metadata from Terraform outputs and update the Project's component tables (cluster endpoint, ArgoCD URL, database endpoints, etc.).
...
5. Terraform state is preserved in S3-compatible storage, so the retry continues from the last successful state
