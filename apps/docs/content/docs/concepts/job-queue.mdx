...
| `DEPLOY_RUNNER` | Provision Runner infrastructure (ECS task, IAM roles) | Adding a cloud-hosted Runner |
| `UPDATE_RUNNER` | Update Runner to latest release | Clicking "Update" on a Runner |
| `DESTROY_RUNNER` | Tear down Runner infrastructure | Removing a cloud-hosted Runner |
...
Job status changes are broadcast via [Postgres LISTEN/NOTIFY → SSE](/concepts/realtime). The UI subscribes to `jobs` UPDATE events and updates the jobs store in real-time. The job detail page also subscribes to `provision_job_logs` INSERT events for live log streaming.
