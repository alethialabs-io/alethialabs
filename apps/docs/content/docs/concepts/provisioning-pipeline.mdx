...
Terraform state is stored in Supabase S3. The backend key follows the pattern `{zone_id}/{project}-{env}-{region}/terraform.tfstate`. See [Terraform State](/concepts/terraform-state).
