...
description: How vine form values flow through config snapshots and provider-specific mappings into Terraform variables.
---
...
The vine form in Trellis (or `alethia plan` in the CLI) calls `createVine()`, which inserts the configuration into normalized component tables:
...
The `VineConfig` struct in `packages/core/types/vine_config.go` mirrors the snapshot shape with typed fields: `Network`, `Cluster`, `DNS`, `Databases[]`, `Caches[]`, and so on.
...
Terraform state is stored in Supabase S3. The backend key follows the pattern `{vineyard_id}/{project}-{env}-{region}/terraform.tfstate`. See [Terraform State](/concepts/terraform-state).
...
### Post-Apply
After apply, the Tendril extracts Terraform outputs (cluster endpoint, database endpoints, ARNs), configures kubeconfig via the provider, installs ArgoCD, and renders application templates. See [GitOps & ArgoCD](/concepts/gitops-argocd).
...
| VineConfig struct | `packages/core/types/vine_config.go` |
