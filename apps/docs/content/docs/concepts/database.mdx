...
| `job_type` | provision_job_type enum | CONNECTION_TEST, PLAN, DEPLOY, DEPLOY_RUNNER, etc. |
| `status` | provision_job_status enum | QUEUED → SUCCESS/FAILED |
| `runner_id` | uuid (FK → runners) | Assigned Runner |
| `config_snapshot` | jsonb | Frozen Spec config for the job |
...
Live streaming: an `AFTER INSERT` trigger emits Postgres `NOTIFY`; the console relays it to the browser over SSE, filtered by `job_id`.
...
|--------|------|-------------|
| `id` | uuid (PK) | |
| `user_id` | uuid (FK) | Owner (null for cloud-hosted) |
| `name` | text | Display name |
| `mode` | runner_mode enum | self-hosted / cloud-hosted |
...
[Access Control](/access-control) for how they're used (the community PDP reads these directly; the
enterprise tier can compile the same model into OpenFGA).

### `permission`

| Column | Type | Description |
|--------|------|-------------|
| `key` | text (PK) | e.g. `spec:deploy` |
...
Parent/child resource edges (e.g. zone → spec) so a grant on a parent cascades.
...
-- Runner status
CREATE TYPE runner_mode AS ENUM ('self-hosted', 'cloud-hosted');
