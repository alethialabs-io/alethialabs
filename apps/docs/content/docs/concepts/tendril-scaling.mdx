...
A user clicks "Plan," "Apply," or "Destroy" in Trellis (or runs `alethia plan` / `alethia harvest` / `alethia destroy`). A `provision_jobs` row is inserted with status `QUEUED`.
