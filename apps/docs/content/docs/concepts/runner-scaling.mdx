...
The Runner authenticates with Console using its worker token, enters its poll loop, and claims the queued job atomically via `FOR UPDATE SKIP LOCKED`. See [Job Queue Pattern](/concepts/job-queue) for claiming details.
