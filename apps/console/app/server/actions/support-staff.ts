...
// dashboard. Every action gates on assertStaff() (the SUPPORT_STAFF_EMAILS allowlist is
// the trust boundary; staff aren't org members, so there's no PDP grant to check) and
// writes via getServiceDb() (RLS-bypass — staff act on every org's cases). Customer-facing
// side effects reuse the built notifiers in lib/support/staff-notify.ts.
...
export async function postStaffReply(
	...
	if (!data.isInternal) {
		const nextStatus = nextStatusAfterStaffReply(caseRow.status);
