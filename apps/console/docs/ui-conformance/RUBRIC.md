...
`components/evidence/{evidence-client,evidence-filter-bar,evidence-query}.tsx` plus
...
`apps/console/docs/ui-conformance/scoreboard.md` is generated by `pnpm -F console audit:report` and
CI diff-gates it, the same contract `PROGRAMME.md` uses. Never hand-edit it.
