...
export function runnerBusyKind(
	...
	if (t === "DEPLOY_RUNNER" || t === "UPDATE_RUNNER" || t === "DESTROY_RUNNER") {
		return t;
	}
	return null;
}
...
export function RunnerVersion({ runner }: { runner: RunnerRow }) {
	...
	return (
		<span className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
			...
			{isOutdated && (
				<ReleaseNotesPopover version={latestRelease.version} runnerId={runner.id} isOutdated>
...
export function RunnerActions({ runner }: { runner: RunnerRow }) {
	...
	const hasCloudResources = !!runner.cloud_identity_id && !!runner.metadata?.deploy_config;
	...
	const handleDestroy = async (assignedRunnerId: string | null) => {
		setDestroying(true);
		try {
			const { jobId } = await destroyRunner({ runnerId: runner.id, assignedRunnerId });
			toast.success("Destroy job queued", {
				action: { label: "View job", onClick: () => router.push(`/dashboard/jobs/${jobId}`) },
			});
		} catch (err) {
	...
	if (busy && runner.activeJob) {
		const label =
			busy === "DEPLOY_RUNNER" ? "Provisioning…" : busy === "UPDATE_RUNNER" ? "Updating…" : "Destroying…";
		return (
			<Link
				...
				<>
					...
					<AlertDialog open={showRemoveDialog} onOpenChange={(open) => !open && setShowRemoveDialog(false)}>
						<AlertDialogContent onClick={(e) => e.stopPropagation()}>
							<AlertDialogHeader>
								<AlertDialogTitle>Remove runner &ldquo;{runner.name}&rdquo;?</AlertDialogTitle>
								<AlertDialogDescription>
