...
// M1: a spec owns N environments, each independently provisionable. This tab lists
...
export function EnvironmentsTab({
	...
	const [environments, setEnvironments] = useState<SpecEnv[]>(initial);
	const [addOpen, setAddOpen] = useState(false);
	const [busyId, setBusyId] = useState<string | null>(null);

	/** Re-pulls the spec's environments after a mutation. */
	async function refresh() {
		try {
			const { environments: rows } = await getSpecEnvironments(specId);
			setEnvironments(rows as SpecEnv[]);
		} catch {
	...
	async function provision(
		...
		try {
			const { jobId } =
				kind === "plan"
					? await planSpec(specId, runnerId, envId)
					: await provisionSpec(specId, undefined, runnerId, envId);
	...
	async function remove(env: SpecEnv) {
		if (!confirm(`Delete the "${env.name}" environment?`)) return;
		setBusyId(env.id);
		try {
			await deleteEnvironment(specId, env.id);
...
function AddEnvironmentDialog({
	...
	async function submit() {
		...
		setSubmitting(true);
		try {
			await addEnvironment(specId, { name, stage });
