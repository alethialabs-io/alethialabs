...
// The spec-detail body, parameterized by ids so BOTH the legacy UUID route
// (`/dashboard/zones/[id]/specs/[specId]`) and the C2 slug route
// (`/{org}/{zone}/{spec}[/{env}]`) render it. The optional `envName` is the
// environment focused by the URL (drives the header status + the Environments tab).
...
interface SpecDetailViewProps {
	zoneId: string;
	specId: string;
	/** The environment focused by the URL (`/{org}/{zone}/{spec}/{env}`), if any. */
	envName?: string;
}

export function SpecDetailView({ zoneId, specId, envName }: SpecDetailViewProps) {
	const router = useRouter();
	const { removeSpec } = useZonesStore();
	const [detail, setDetail] = useState<SpecData | null>(null);
	...
	// The environment the URL focuses (else the spec's default), for the header.
	const activeEnv =
		detail.environments?.find((e) => e.name === envName) ??
		detail.environments?.find((e) => e.is_default) ??
		null;
	const headEnvName = activeEnv?.name ?? spec.environment_stage;
	const headStatus = activeEnv?.status ?? spec.status;
				...
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<p className="text-xs text-muted-foreground">
							{meta.shortName} · {spec.region} · {headEnvName}
