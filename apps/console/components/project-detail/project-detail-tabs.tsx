...
export type ProjectDetail = {
	...
	// M1: the project's environments (the default + any added). Drives the Environments tab.
	environments?: {
		id: string;
		name: string;
		stage: string;
...
const projectJobColumns: ColumnDef<JobWithMeta>[] = [
	{
...
export function ProjectDetailTabs({ detail, projectId, plan, onApplied }: ProjectDetailTabsProps) {
	...
	const { project, cloudProvider, components } = detail;
	...
	return (
		<Tabs value={activeTab} onValueChange={setTab}>
			...
			<TabsContent value="config" className="mt-6">
				<div className="space-y-8">
					...
					<div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
						...
						<StatChip icon={<DollarSign className="h-3.5 w-3.5" />} value={project.estimated_monthly_cost ? `$${Math.round(project.estimated_monthly_cost)}` : "—"} label="Est. Monthly" />
						<StatChip icon={<CheckCircle2 className="h-3.5 w-3.5" />} value={project.status} label="Status" />
						<StatChip icon={<Globe className="h-3.5 w-3.5" />} value={`${providerMeta.shortName} · ${project.region}`} label="Provider" />
					</div>
					...
					{jobs.length > 0 && (
						<section className="space-y-3">
							...
							<DataTable columns={projectJobColumns} data={jobs} onRowClick={handleJobClick} pageSize={5} />
						</section>
					)}
				</div>
			</TabsContent>
			...
			<TabsContent value="environments" className="mt-6">
				<EnvironmentsTab projectId={projectId} environments={detail.environments ?? []} />
			</TabsContent>
