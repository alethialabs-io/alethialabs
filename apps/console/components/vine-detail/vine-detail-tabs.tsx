...
export function VineDetailTabs({ detail, vineId, plan, onApplied }: VineDetailTabsProps) {
	...
	const { vine, cloudProvider, components } = detail;
			...
			<TabsContent value="config" className="mt-6">
				<div className="space-y-8">
					...
					<div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
						...
						<StatChip icon={<DollarSign className="h-3.5 w-3.5" />} value={vine.estimated_monthly_cost ? `$${Math.round(vine.estimated_monthly_cost)}` : "—"} label="Est. Monthly" />
						<StatChip icon={<CheckCircle2 className="h-3.5 w-3.5" />} value={vine.status} label="Status" />
						<StatChip icon={<Globe className="h-3.5 w-3.5" />} value={`${providerMeta.shortName} · ${vine.region}`} label="Provider" />
					</div>
					...
					{jobs.length > 0 && (
						<section className="space-y-3">
							...
							<DataTable columns={vineJobColumns} data={jobs} onRowClick={handleJobClick} pageSize={5} />
						</section>
					)}
				</div>
			</TabsContent>
