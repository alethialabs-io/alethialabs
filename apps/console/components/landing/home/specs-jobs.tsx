...
const FLOW: [IconKey, string, string][] = [
	["layers", "Author a Spec", "Eleven guided sections — network, cluster, databases, caches, DNS, secrets."],
...
/** 03 · Specs → jobs — the lifecycle from a saved Spec to a reconciled cluster. */
export function SpecsJobs() {
	return (
		<section style={{ padding: "84px 0", borderTop: "1px solid var(--border)" }}>
			<Wrap>
				<SecMark n="03" label="Specs → jobs" />
				<h2 style={{ ...disp, fontSize: 38, fontWeight: 600, letterSpacing: "-0.035em", margin: "0 0 14px", maxWidth: 620, color: "var(--text-primary)" }}>From a saved Spec to a reconciled cluster.</h2>
				<p style={{ fontSize: 15.5, color: "var(--text-tertiary)", maxWidth: 560, margin: "0 0 44px", lineHeight: 1.6 }}>Apply a Spec and the platform compiles it, queues a job, runs it on a runner, and hands the cluster to GitOps — every step streamed and auditable.</p>
