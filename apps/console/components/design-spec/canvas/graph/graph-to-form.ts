...
/**
 * Projects the canvas graph onto the SpecFormData shape so it persists through the
 * SAME `createSpec` the form uses. Returns a plain object validated by
 * `specFormSchema.safeParse` at save time (missing required nodes surface there).
 *
 ...
 * inherits the spec's primary identity.
 */
