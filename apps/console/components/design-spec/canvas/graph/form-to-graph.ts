...
/**
 * Seeds the canvas from a SpecFormData object (initial mount / source spec / the
...
export function formToGraph(
	...
	const coreId = form.spec.cloud_identity_id || null;
	...
	const nodes: CanvasNode[] = [
		makeNode(
			"project",
			{
				project_name: form.spec.project_name,
				environment_stage: form.spec.environment_stage,
				region: form.spec.region,
				zone_id: form.spec.zone_id,
				iac_version: form.spec.iac_version,
			},
			{ x: 260, y: 0 },
			coreId,
		),
