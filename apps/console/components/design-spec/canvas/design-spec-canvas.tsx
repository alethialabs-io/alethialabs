...
function CanvasInner({ cloudIdentities, onToggleForm }: DesignSpecCanvasProps) {
	...
	const handleSave = useCallback(async () => {
		...
		const parsed = specFormSchema.safeParse(graphToForm(nodes));
		if (!parsed.success) {
			const first = parsed.error.issues[0];
			toast.error(
				`Can't save: ${first?.path.join(".") || "spec"} — ${first?.message}`,
			);
			return;
		}
		setSaving(true);
		try {
			const { spec } = await createSpec(
				parsed.data as unknown as CreateSpecInput,
			);
			toast.success("Spec created!");
			useCanvasStore.getState().reset();
			router.push(
				spec.zone_id
					? `/dashboard/zones/${spec.zone_id}/specs/${spec.id}`
					: "/dashboard/specs",
			);
		} catch (e) {
