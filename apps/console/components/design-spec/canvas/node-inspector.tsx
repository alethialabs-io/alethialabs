						...
						<SheetHeader>
							...
							<SheetDescription className="text-xs">
								{def.classification === "root"
									? "Spec basics and the stack's core cloud account."
									: def.classification === "core"
							...
							{gated && (
								<Alert variant="default" className="border-border bg-muted">
									<AlertDescription className="text-xs">
										...
										drawn, but the spec will not provision until colocated.
									</AlertDescription>
								</Alert>
							)}
	...
	if (node.data.kind === "project") {
		return (
			<>
				...
				<Field label="Zone">
					<ZoneSelector
						value={(c.zone_id as string) || undefined}
						onChange={(v) => onChange({ zone_id: v || "" })}
					/>
				</Field>
