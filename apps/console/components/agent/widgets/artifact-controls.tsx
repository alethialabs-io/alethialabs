...
export function specFromWidgets(widgets: ThreadWidget[]): ArtifactSpec {
	const minY = widgets.reduce((m, w) => Math.min(m, w.pos_y), Number.POSITIVE_INFINITY);
	return {
		...
		<Popover open={open} onOpenChange={setOpen}>
			<PopoverTrigger asChild>
				<Button
					...
					aria-label={kind === "widget" ? "Save widget as artifact" : "Save dashboard as artifact"}
			...
			<PopoverContent align="end" className="w-[240px] rounded-none p-2">
				<div className="vx-eyebrow pb-1.5 text-[9px]">
					{kind === "widget" ? "Save widget as artifact" : "Save dashboard as artifact"}
				</div>
				<Input
					value={name}
				...
				{items === null ? (
					...
					<div className="max-h-[240px] overflow-y-auto">
						{items.map((a) => (
							...
							>
								<span title={a.name} className="min-w-0 flex-1 truncate text-[12px] text-foreground">
									{a.name}
								</span>
