...
// The internal Alibaba-token route makes managed Alibaba provisioning keyless: an authenticated runner
// gets a freshly-minted OIDC assertion it hands (via a token file) to the alicloud provider, which runs an
// anonymous AssumeRoleWithOIDC — no AccessKey. These tests pin the contract: a real minted token that
// verifies against the published JWKS (audience sts.aliyuncs.com), 501 when the issuer isn't configured,
// and 401 when the runner isn't authenticated.
...
async function post() {
	const { POST } = await import("@/app/api/runners/alibaba-token/route");
	return POST(new Request("https://console.local/api/runners/alibaba-token", { method: "POST" }));
}

describe("POST /api/runners/alibaba-token", () => {
	it("mints an Alibaba assertion that verifies against the JWKS", async () => {
		installKey();
