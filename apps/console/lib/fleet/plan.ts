...
// The Fleet Controller's brain: a PURE function that diffs a pool project against observed
...
export function targetCount(project: FleetTarget, backlog: number, recentPeak: number): number {
	const slots = Math.max(1, project.slotsPerRunner);
	const demand = Math.max(recentPeak, Math.ceil(Math.max(0, backlog) / slots));
	return clamp(Math.max(project.warmMin, demand + project.buffer), 0, project.max);
}
...
export function plan(project: FleetTarget, o: Observed): FleetAction[] {
	...
	function pickLocation(): string {
		for (const loc of project.locations) {
			if ((liveByLoc.get(loc) ?? 0) < project.minPerLocation) return loc;
		}
		let best = project.locations[0] ?? "";
	...
	const outdated = online.filter((h) => h.version !== project.targetVersion);
	if (project.targetVersion !== null && outdated.length > 0) {
	...
	if (outdated.length === 0 && o.surplusTicks >= project.scaleDownGraceTicks) {
