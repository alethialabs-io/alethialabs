...
interface ArtifactState {
	...
	tab: ArtifactTab;
	/** Open an artifact in the panel; defaults the tab to Config (project) or Logs (job-only). */
...
/**
 * Holds the agent page's active artifact (a project and/or a job). Tool-result cards
...
export const useArtifactStore = create<ArtifactState>((set) => ({
	...
	tab: "config",
	open: (artifact, tab) =>
		set({ artifact, tab: tab ?? (artifact.projectId ? "config" : "logs") }),
	setTab: (tab) => set({ tab }),
