...
/** Filters supported against `project_full` (all AND-combined, all optional). */
interface ProjectFullFilters {
	id?: string;
	user_id?: string;
	project_name?: string;
}
...
/**
 * Reads rows from the `project_full` view with the given equality filters. Returns the
 * raw view rows typed as ProjectFull[] (caller decides single/array/not-found). Service
 * path only — the 4 consumers are all CLI/runner endpoints on getServiceDb().
 */
export async function queryProjectFull(
	db: ServiceDb,
	filters: ProjectFullFilters,
): Promise<ProjectFull[]> {
	const conds: SQL[] = [];
	...
	return db.execute<ProjectFull>(sql`select * from project_full${where}`);
}
