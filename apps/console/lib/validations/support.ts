...
// Validation for the support-cases feature. drizzle-zod derives the case-submit
// contract from the supportCases schema (JSONB columns refined with their interface
// types); the message + label maps are hand-authored zod / display copy shared by the
// submit dialog + thread (client) and the server actions (which re-validate).
