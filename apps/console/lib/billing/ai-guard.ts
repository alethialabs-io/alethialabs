 ...
 *
 * When `userId` is supplied, an additional **per-seat** daily + weekly sub-cap is enforced
 * on top of the org caps (a fraction of the org allowance — see `AiTierSpec.perUser*`), so
 * one member can't drain the whole workspace's included budget. A seat that has exhausted
 * its personal share while the org still has room is blocked (buying org credits doesn't
...
export async function assertAiAllowed(
	...
	// block THIS seat (don't silently divert to purchased packs). Buying org credits or
	// upgrading the org plan doesn't lift a personal cap — it clears on the bucket reset,
	// or an admin raises the per-seat limit. Fail-closed.
	if (orgDayOk && orgWeekOk && (!userDayOk || !userWeekOk)) {
		const weeklyHit = !userWeekOk;
		throw new AiBudgetError(
			weeklyHit
	...
	// Org included budget exhausted for this bucket — spend purchased top-ups if any, unless
	// the org's hard-cap policy says to pause at the included allowance instead.
