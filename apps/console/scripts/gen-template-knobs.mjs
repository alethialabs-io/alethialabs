...
//
// Run: `pnpm -F console gen:template-knobs` (writes), `check:template-knobs` (asserts + regenerates).
...
const manifest = {
	...
	// is edit it.
	generatedBy: "apps/console/scripts/gen-template-knobs.mjs — do not edit; run `pnpm -F console gen:template-knobs`",
	clouds: CLOUDS,
...
function renderDoc() {
	...
	L.push("");
	L.push("<!-- GENERATED by apps/console/scripts/gen-template-knobs.mjs. Do not edit; run `pnpm -F console gen:template-knobs`. -->");
	L.push("");
	L.push(
...
if (process.argv[1]?.endsWith("gen-template-knobs.mjs")) {
	...
	if (process.argv.includes("--check")) {
		...
		if (stale.length) {
			console.error(`gen:template-knobs --check: stale generated file(s): ${stale.join(", ")}. Run \`pnpm -F console gen:template-knobs\` and commit.`);
			process.exit(1);
		}
