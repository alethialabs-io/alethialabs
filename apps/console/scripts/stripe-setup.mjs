...
//
// What it ensures:
//   • Product "Alethia Pro" + per-seat Price ($29/seat/mo)        → STRIPE_PRICE_TEAM
...
//   STRIPE_SECRET_KEY=sk_live_… node scripts/stripe-setup.mjs --webhook-url=https://alethialabs.io/api/webhooks/stripe
//
...
// ── Pricing (keep in sync with @repo/plan-catalog priceLabel + lib/billing/{plan,meter}.ts) ──
const PRO_UNIT_AMOUNT = 2900; // $29.00 / seat / month
 ...
 *  explicit `STRIPE_SECRET_KEY=sk_live_… node …` wins over the file's test key). */
...
async function ensureProduct(stripe, tag, name) {
...
async function main() {
	...
	console.log(`→ Stripe ${mode} mode — ensuring catalog…\n`);

	// 1) Pro product + per-seat price.
	const proProduct = await ensureProduct(stripe, "pro", "Alethia Pro");
	const proPrice = await ensurePrice(stripe, LK_PRO, {
		product: proProduct.id,
		currency: "usd",
		unit_amount: PRO_UNIT_AMOUNT,
		recurring: { interval: "month", usage_type: "licensed" },
		nickname: "Alethia Pro — $29 / seat / mo",
	});
	console.log(`✓ Pro product ${proProduct.id}`);
	console.log(`✓ Pro price   ${proPrice.id}  (${LK_PRO})`);
	...
	const runnerProduct = await ensureProduct(
		stripe,
		"runner_minutes",
		"Alethia runner minutes",
	);
	const meterPrice = await ensurePrice(stripe, LK_METER_PRO, {
		...
		tiers: [
			...
			{ up_to: "inf", unit_amount_decimal: RUNNER_OVERAGE_CENTS },
		],
	...
	if (webhookUrl) {
		...
		if (match) {
			console.log(
				`\n⚠ Webhook endpoint already exists (${match.id}). Its signing secret is only` +
					` shown at creation — roll it in the dashboard if you need a fresh STRIPE_WEBHOOK_SECRET.`,
			);
		} else {
