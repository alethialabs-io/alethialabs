...
const EMAIL = "alerts-smoke@example.com";
...
async function main() {
  ...
  try {
    ...
    await page
      ...
      .fill(`http://localhost:${CATCHER_PORT}/hook`);
    ...
    check(
      "Test delivery reached the endpoint (live verifyChannel)",
      hits.length > before,
      `hits ${hits.length}`,
    );
    ...
    const [ch] = await db
      ...
      .limit(1);
    check(
      "channel persisted with org scope",
      Boolean(ch?.org_id),
      `org=${ch?.org_id?.slice(0, 8)}`,
    );
    const [rule] = await db
      .insert(alertRules)
      .values({
    ...
    check(
      "emit(system.job.failed) created a delivery",
      n >= 1,
      `deliveries ${n}`,
    );
    ...
    await db
      ...
      .where(eq(alertDeliveries.channel_id, ch.id));
    await db
