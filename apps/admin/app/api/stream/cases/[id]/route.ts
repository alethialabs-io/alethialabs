...
import { supportCases, supportMessages } from "@repo/support/schema";
 ...
 * (not org membership), reads via getServiceDb, and — unlike a customer stream — emits ALL
 * messages INCLUDING is_internal staff notes. Backlog since Last-Event-ID, then live
...
export async function GET(
	...
	const stream = new ReadableStream({
		async start(controller) {
			...
			unsubscribe = getSupportMessageTransport().subscribe(
				caseId,
				(messageId) => {
					if (closed) return;
					void (async () => {
