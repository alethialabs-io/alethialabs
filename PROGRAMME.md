...
**3 of 25 proof cells are proven.** 2 failing · 0 stale (cause fixed, needs a re-run) · 0 blocked · 20 never run.
...
Ledger rows read: **19** · surviving claims: **5** (a `RETRACTED` row voids a claim rather than replacing it, so surviving < rows is expected).
