...
**16 of 30 proof cells are proven.** 4 failing · 2 stale (cause fixed, needs a re-run) · 0 blocked · 8 never run.
...
Ledger rows read: **53** · surviving claims: **22** (a `RETRACTED` row voids a claim rather than replacing it, so surviving < rows is expected).
