...
**10 of 25 proof cells are proven.** 7 failing · 1 stale (cause fixed, needs a re-run) · 0 blocked · 7 never run.
...
Ledger rows read: **33** · surviving claims: **16** (a `RETRACTED` row voids a claim rather than replacing it, so surviving < rows is expected).
