...
**2 of 25 proof cells are proven.** 1 failing · 0 stale (cause fixed, needs a re-run) · 0 blocked · 22 never run.
...
Ledger rows read: **17** · surviving claims: **3** (a `RETRACTED` row voids a claim rather than replacing it, so surviving < rows is expected).
