...
**7 of 25 proof cells are proven.** 5 failing · 1 stale (cause fixed, needs a re-run) · 0 blocked · 12 never run.
...
Ledger rows read: **28** · surviving claims: **11** (a `RETRACTED` row voids a claim rather than replacing it, so surviving < rows is expected).
