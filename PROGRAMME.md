...
**18 of 30 proof cells are proven.** 4 failing · 1 stale (cause fixed, needs a re-run) · 0 blocked · 7 never run.
...
Ledger rows read: **55** · surviving claims: **23** (a `RETRACTED` row voids a claim rather than replacing it, so surviving < rows is expected).
