...
**11 of 30 proof cells are proven.** 4 failing · 3 stale (cause fixed, needs a re-run) · 0 blocked · 9 never run.
...
Ledger rows read: **48** · surviving claims: **21** (a `RETRACTED` row voids a claim rather than replacing it, so surviving < rows is expected).
