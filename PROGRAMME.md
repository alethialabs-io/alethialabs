...
**21 of 30 proof cells are proven.** 0 failing · 1 stale (cause fixed, needs a re-run) · 0 blocked · 8 never run.
...
Ledger rows read: **58** · surviving claims: **24** (a `RETRACTED` row voids a claim rather than replacing it, so surviving < rows is expected).
