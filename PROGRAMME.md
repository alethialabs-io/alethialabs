...
**10 of 25 proof cells are proven.** 6 failing · 1 stale (cause fixed, needs a re-run) · 0 blocked · 8 never run.
...
Ledger rows read: **32** · surviving claims: **15** (a `RETRACTED` row voids a claim rather than replacing it, so surviving < rows is expected).
