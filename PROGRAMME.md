...
**4 of 25 proof cells are proven.** 1 failing · 1 stale (cause fixed, needs a re-run) · 1 blocked · 18 never run.
...
Ledger rows read: **21** · surviving claims: **7** (a `RETRACTED` row voids a claim rather than replacing it, so surviving < rows is expected).
