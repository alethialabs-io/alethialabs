...
**20 of 30 proof cells are proven.** 0 failing · 1 stale (cause fixed, needs a re-run) · 0 blocked · 9 never run.
...
Ledger rows read: **57** · surviving claims: **23** (a `RETRACTED` row voids a claim rather than replacing it, so surviving < rows is expected).
