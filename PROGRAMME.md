...
**9 of 30 proof cells are proven.** 4 failing · 4 stale (cause fixed, needs a re-run) · 0 blocked · 10 never run.
...
Ledger rows read: **46** · surviving claims: **20** (a `RETRACTED` row voids a claim rather than replacing it, so surviving < rows is expected).
