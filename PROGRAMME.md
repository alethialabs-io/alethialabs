...
**8 of 25 proof cells are proven.** 5 failing · 1 stale (cause fixed, needs a re-run) · 0 blocked · 11 never run.
...
Ledger rows read: **29** · surviving claims: **12** (a `RETRACTED` row voids a claim rather than replacing it, so surviving < rows is expected).
