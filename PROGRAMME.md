...
**9 of 25 proof cells are proven.** 6 failing · 0 stale (cause fixed, needs a re-run) · 0 blocked · 10 never run.
...
**`aws/maxconfig`** — failing. ledger 2026-08-25 (via the `full` composite run)
...
Ledger rows read: **31** · surviving claims: **13** (a `RETRACTED` row voids a claim rather than replacing it, so surviving < rows is expected).
