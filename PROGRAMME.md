...
**0 of 25 proof cells are proven.** 2 failing · 1 stale (cause fixed, needs a re-run) · 0 blocked · 22 never run.
...
- `aws/floor` **failing** — ledger 2026-08-24
- `hetzner/floor` **failing** — ledger 2026-08-24
...
**`hetzner/addons`** — stale. ledger 2026-08-05 — but #2058 is CLOSED, so the cause is fixed and this needs a fresh run, not a fix
...
|---|---|---|:---:|
| `aws/floor` | failing | **none** | ? |
| `hetzner/floor` | failing | **none** | ? |
...
Ledger rows read: **14** · surviving claims: **3** (a `RETRACTED` row voids a claim rather than replacing it, so surviving < rows is expected).
