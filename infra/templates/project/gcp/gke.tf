...
module "gke" {
  ...
  network_name          = var.provision_network ? module.vpc_network[0].network_name : one(data.google_compute_network.existing[*].name)
  subnet_name           = var.provision_network ? module.vpc_network[0].private_subnet_name : one(data.google_compute_subnetwork.existing[*].name)
