...
| `encryption_enabled` | **Not accepted** (deleted by #4320) — Hetzner Object Storage supports only SSE-C (per-request customer keys) and has no bucket-level default-encryption configuration, so there is nothing to write. Objects are encrypted at rest either way. |
