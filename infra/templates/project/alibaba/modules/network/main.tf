...
locals {
  # Discovered zone ids to spread vswitches across (may be unknown at plan). NOTE: never take
  # length(local.zones) for a count — that's unknown at plan under the runner's deferred RAM-OIDC
  # provider. The vswitch count is the plan-known var.subnet_count; we element()-index this list.
  zones = var.zone_ids
}
...
# One vswitch (subnet) per slot. Count is a plan-known static (var.subnet_count); the zone is
# element()-selected from the discovered zone list, wrapping when the region has fewer zones.
...
resource "alicloud_snat_entry" "this" {
  count = var.single_cloud_nat ? var.subnet_count : 0
