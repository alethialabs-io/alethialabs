...
## Runtime Errors (worker running in Fargate)

### Worker Goes Offline

**Detection:** Trellis checks `last_heartbeat` on workers table.
...
- Worker re-enters poll loop on restart, sends heartbeat immediately
- Stale jobs auto-recovered by `recover_stale_jobs()` RPC after 15 minutes

**User-visible:**
- Trellis Workers page shows status `OFFLINE` with last heartbeat time
- Jobs stuck in `CLAIMED`/`PROCESSING` get reset to `QUEUED` after 15 minutes
- Future: push notification to user when worker goes offline

### Job Fails

**Detection:** Worker catches error, sets job status to `FAILED` with error message.
...
2. Worker's heartbeat stops
3. After 15 minutes, `recover_stale_jobs()` resets the job to `QUEUED`
4. Another worker (or the restarted same worker) picks it up

**Risk:** Terraform state might be inconsistent if the worker died mid-apply. The re-run will attempt to reconcile via Terraform's state file (stored in S3).
...
**Workers page:**
- Worker name, mode, status (ONLINE/OFFLINE/DRAINING), last heartbeat, created date
- Visual indicator: green dot for ONLINE, grey for OFFLINE, yellow for DRAINING

**Jobs table:**
- Job type, status (QUEUED/CLAIMED/PROCESSING/SUCCESS/FAILED), worker, created, completed
...
- Useful for debugging worker-level issues (vs job-level issues in Trellis)

### Future: Health Alerts

- Trellis detects worker OFFLINE for > 10 minutes → email/notification
