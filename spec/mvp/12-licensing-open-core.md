...
- Alethia is a **tool that operates infrastructure**, not a library linked into the customer's product. The §13 obligation lands on whoever **modifies and hosts Alethia**, not on the customer's applications it provisions. The plain-HTTP runner boundary (`packages/core/api/api.go`, Bearer over `ALETHIA_WEB_ORIGIN`) reinforces this — the customer's workloads never link Alethia code.
