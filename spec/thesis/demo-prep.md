...
- [ ] Tendril вече деплойнат и ONLINE (за plan/apply по време на демото)
- [ ] Verify tendril е scaled to 0 (за да демонстрираш scale-to-zero при нужда)
...
**Какво казваш:** "За GCP използваме Workload Identity Federation. Потребителят конфигурира WIF pool в своя GCP проект. Нашият Tendril представя OIDC токен, GCP го валидира и издава временни credentials. Няма service account JSON ключове."

**Ако те питат: Какво е Workload Identity Federation?**
WIF позволява external identity providers (в нашия случай — Trellis/Supabase) да се автентикират в GCP без service account ключове. Потребителят създава Workload Identity Pool и Provider в GCP конзолата. Конфигурира се OIDC issuer URL и audience. Когато Tendril трябва достъп, той извиква `STS:ExchangeToken` с OIDC токен от Trellis, GCP го валидира и връща short-lived access token. Няма файлове, няма ключове, няма какво да leak-не.
...
Потребителят създава App Registration в Azure AD (сега Entra ID). Добавя Federated Credential с issuer URL (Trellis) и subject claim. Когато Tendril трябва достъп, той представя OIDC токен, Azure го валидира срещу registered issuer/subject, и издава access token. Никога не се генерира client secret. Ако потребителят иска да отнеме достъпа — просто трие App Registration-а.
...
2. Създават се child rows в component таблиците: `vine_network`, `vine_cluster`, `vine_databases`, `vine_caches`, etc.
...
Viticulture (лозарство) тема: Trellis = опора (web), Grape = грозде (CLI), Vineyard = лозе (workspace/project), Vine = лоза (infrastructure configuration), Tendril = филиз (execution agent), Harvest = бране (deployment). Не е просто маркетинг — помага за запомняне на компонентите.
...
### Какво показваш
Отиваш на vine detail страницата. Показваш Infrastructure tab-а. Натискаш Plan → отваря се Tendril Select Popover.

### Какво казваш
"Сега имаме конфигурация. Натискам Plan — и тук се отваря tendril selector. Може да си зададете въпроса — как точно работи това? Кой изпълнява Terraform? Ако нямаме статични ключове, как достъпваме AWS акаунта на потребителя?"

**Pause — обръщаш се към комисията:**
"Сега ще покажа."

Избираш cloud tendril → job се създава (QUEUED).

### Ако те питат: Какво е Tendril Select Popover?
Компонент, който показва всички регистрирани Tendril-и и техния статус (ONLINE/OFFLINE/DRAINING). Потребителят избира кой Tendril да изпълни задачата, или "Any available" за автоматичен избор. Зеленото кръгче = heartbeat получен в последните 60 секунди.

---

## Стъпка 6: Tendrils — обяснение и създаване — ~1.5 мин

### Какво показваш
Отиваш на Tendrils страницата. Обясняваш какво е Tendril. Създаваш нов Tendril.

### Какво казваш
"Tendrils са Go контейнери, работещи в ECS Fargate — Amazon's serverless container service. Tendril-ът е нашият execution engine."

"Ето какво прави Tendril-ът:
- На всеки 10 секунди poll-ва за задачи чрез POST /api/jobs/claim
- Поемането е атомарно — функцията claim_next_job() в PostgreSQL гарантира, че една задача не се изпълнява от два Tendril-а
...
**Scale-to-zero:**
"Когато няма задачи в опашката за 5 минути, Lambda scaler мащабира ECS сервиса до 0. Не плащаме за Fargate, когато няма работа. При нова задача — автоматично обратно на 1, cold start от около 50 секунди."

Показваш "Add Tendril" → избираш регион, CPU, memory → Create.

"Сега се създава DEPLOY_RUNNER job. Tendril infrastructure-та се провизира автоматично — ECS task definition, ECR image, IAM roles, CloudWatch log group. След около 2 минути ще видим нов Tendril със статус ONLINE."

### Ако те питат: Защо ECS Fargate?
Fargate е serverless — не управляваме EC2 instances. Плащаме само когато Tendril работи. Scaling е автоматичен. Не трябва да пачваме OS или runtime. Tendril-ът е Docker image с Go binary + Terraform binary + kubectl + Helm — всичко embedded.
...
3. Записва runner_id
4. Връща задачата на Tendril-а

Всичко е в една транзакция с `FOR UPDATE SKIP LOCKED`. Ако два Tendril-а poll-нат едновременно, само единият получава задачата. Няма race condition, няма дублиране.

### Ако те питат: Как работи heartbeat?
Tendril изпраща POST /api/runners/heartbeat на всеки 30 секунди. Trellis записва timestamp-а. Ако timestamp-ът е по-стар от 60 секунди, Tendril-ът се маркира OFFLINE. Ако Tendril умре по средата на задача, Trellis requeue-ва задачата и друг Tendril може да я поеме. Това е graceful recovery — без manual intervention.

### Ако те питат: Как работи scale-to-zero?
Lambda функция се тригерва от EventBridge на всяка минута. Проверява Supabase за QUEUED jobs. Ако има queued jobs и ECS desired count е 0 — scale up до 1. Ако няма queued jobs, инкрементира idle counter. При 5 последователни idle проверки (5 минути) — scale down до 0. Така плащаме за Fargate само когато има работа. Lambda-та струва ~$0.02/месец, а спестява ~$40/месец за неизползван Tendril.

### Ако те питат: Как се версионират компонентите?
Grape и Tendril се версионират чрез release-please с conventional commits. `grape-v*` тагове тригерват GoReleaser за CLI бинарки + Homebrew. `tendril-v*` тагове тригерват Docker build за ARM64 + push до ECR и GHCR + force-new-deployment на ECS. Trellis получава release metadata чрез POST /api/releases/runner и го показва в UI-то.

### Ако те питат: Self-hosted vs Cloud-hosted?
Два режима:
- **Cloud-hosted:** Платформата деплойва Tendril в Fargate. Потребителят не инсталира нищо. Scale-to-zero е автоматичен.
- **Self-hosted:** Потребителят пуска `tendril` binary-то на своя машина или в свой клъстер. Tendril-ът е същият Go binary, но работи в инфраструктурата на потребителя. Полезно за on-prem или air-gapped environments.
...
Tendril изпълнява `terraform plan` с конфигурацията от vine-а. Генерира `.tfplan` файл. После `terraform show -json` за structured output. Ако има Infracost token — пуска Infracost analysis на plan JSON-а за cost breakdown. Plan artifact се записва в Supabase S3 bucket.

### Ако те питат: Къде се пази Terraform state?
Supabase S3 backend. Path structure: `{vineyard}/{project}/{stage}/{region}/terraform.tfstate`. Всеки vine има собствен state файл. Backend config-ът се генерира динамично от Tendril-а преди `terraform init`.
...
- DEPLOY_RUNNER → SUCCESS

Отваряш DEPLOY job-а → показваш real-time логове.

### Какво казваш
"Всяка операция е job в опашката. Виждаме статусите — QUEUED означава чака Tendril, PROCESSING означава Tendril работи по нея, SUCCESS/FAILED е крайният резултат."

"Логовете се стриймват в реално време. Tendril пише Terraform output в batch-ове на всеки 2 секунди, POST-ва ги на /api/jobs/{id}/logs, и Supabase Realtime ги push-ва на клиента чрез WebSocket. Виждаме точно какво прави Terraform в момента."

### Ако те питат: Как работи log streaming?
Tendril има `JobLogger` — Go struct, който буферира stdout/stderr и ги flush-ва на всеки 2 секунди или когато буферът надхвърли 10KB, като batch POST към Trellis API. Trellis записва в `provision_job_logs` таблицата. Клиентът е subscribed чрез Supabase Realtime на INSERT events в тази таблица. Всеки нов batch се появява в UI-то веднага.
...
- `DEPLOY_RUNNER` — провизиране на Tendril infrastructure
- `DESTROY_RUNNER` — премахване на Tendril infrastructure
...
След успешен `terraform apply`, Tendril-ът:
...
### Защо Go за CLI-то и Tendril-а, а не Python/Node/Rust?
...
- **Concurrency** — goroutines за паралелен streaming на логове и heartbeat
...
### Как се гарантира, че два Tendril-а не изпълняват една задача?
PostgreSQL `claim_next_job()` функция. Използва `FOR UPDATE SKIP LOCKED` — row-level lock, който е невидим за другите транзакции. Атомарно: SELECT + UPDATE в една транзакция. Ако Tendril-ът crash-не, lock-ът се освобождава автоматично.

### Какво се случва ако Tendril умре по средата на deploy?
1. Heartbeat спира да идва
2. Trellis маркира Tendril-а OFFLINE след 60 секунди
...
### Как работи scale-to-zero?
Lambda функция (Python 3.12, ARM64) се тригерва от EventBridge на всяка минута. Проверява Supabase REST API за QUEUED jobs. Ако има queued jobs и ECS desired count е 0 — ecs:UpdateService до 1. Ако няма jobs, инкрементира idle counter. При 5 idle проверки (5 минути) — scale down до 0. Multi-region: итерира през всички регистрирани Tendril ECS сервиси. Lambda: ~$0.02/месец. Спестява: ~$40/месец на неизползван Tendril.

### Как се версионират Grape и Tendril?
release-please с conventional commits. Два компонента с отделни PRs: `apps/grape` и `apps/tendril`. При merge release-please създава git тагове (`grape-v*`, `tendril-v*`). Grape: GoReleaser → бинарки за 4 платформи + Homebrew tap. Tendril: Docker buildx ARM64 → ECR + GHCR → POST /api/releases/runner → force-new-deployment на ECS. Автоматични CHANGELOG-ове.
