...
"Сега се създава DEPLOY_WORKER job. Tendril infrastructure-та се провизира автоматично — ECS task definition, ECR image, IAM roles, CloudWatch log group. След около 2 минути ще видим нов Tendril със статус ONLINE."
...
Grape и Tendril се версионират чрез release-please с conventional commits. `grape-v*` тагове тригерват GoReleaser за CLI бинарки + Homebrew. `tendril-v*` тагове тригерват Docker build за ARM64 + push до ECR и GHCR + force-new-deployment на ECS. Trellis получава release metadata чрез POST /api/releases/worker и го показва в UI-то.
...
- `DEPLOY_WORKER` — провизиране на Tendril infrastructure
- `DESTROY_WORKER` — премахване на Tendril infrastructure
