...
function buildAntiProbeTree() {
	...
	put(root, "apps/console/app/(private)/layout.tsx", "export default function PL({ children }) { return <div>{children}</div>; }\n");
	// THE BOUNDARY-ESCAPE ANTIPROBE: the SAME throwing layout as the probe's, with the one file
	// that fixes it — a not-found.tsx at the segment above. There is deliberately no root
	// not-found.tsx here, so the invariant cannot be passing this tree by finding that one. The
	// segment's own not-found.tsx is present too: having one does not make a layout's throw
	// catchable, and having one must not make it un-reportable either.
...
export function positiveControl() {
	...
	try {
		...
		if (p.escapes.throwing.length !== 1 || p.escapes.throwing[0] !== escapeAt) {
			problems.push(
				`probe boundary-escape: expected exactly ${escapeAt} to call notFound(), got ` +
					`[${p.escapes.throwing.join(", ")}] — the layout matcher has stopped matching`,
			);
		}
		if (p.escapes.escapes.length !== 1 || !p.escapes.escapes[0].startsWith(escapeAt)) {
			problems.push(
				`probe boundary-escape: expected one escape at ${escapeAt}, got ` +
					`${p.escapes.escapes.length}: [${p.escapes.escapes.join(" | ")}]`,
			);
		}
		if (a.escapes.throwing.length !== 1) {
			problems.push(
				`antiProbe boundary-escape: the fixed tree must still HAVE a throwing layout, or it ` +
					`proves nothing about the quiet branch — got ${a.escapes.throwing.length}`,
			);
		}
