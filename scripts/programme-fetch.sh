...
# THE BOARD IS PASSED BY FILE, NOT BY ARGV — it outgrew `ARG_MAX` and took the gate with it.
#
# Every large value below used to be an `--argjson` whose JSON was expanded onto jq's command line.
# `execve(2)` caps the total argv+environ at `ARG_MAX`, and on 2026-09-03 the open+closed board
# crossed it: this line began failing with `/usr/bin/jq: Argument list too long` and exit **126**,
# and it failed on every scheduled run afterwards. The snapshot stopped refreshing, went stale, and
# `check-programme-rollup`'s integrity check then RED-ED `Authz / open-core guards` on every pull
# request in the repository — the self-test says so in as many words. One `E2BIG` on a nightly cron
# closed the merge queue for the whole repo, five days before anybody traced it back here.
#
# `--slurpfile` reads the JSON from a file instead, so the size limit is the filesystem's rather
# than the kernel's. It wraps each document in an array, hence the `[0]` on each binding below.
# The small scalars stay on argv: `--arg`/`--argjson` of a date or a boolean cannot approach the cap.
#
# Growth is the normal state of a board, so this is not a one-off repair: the failure returns for
# ANY value that can grow without bound. Keep unbounded values off argv.
args_dir="$(mktemp -d)"
# Both temp dirs in ONE trap: a second `trap ... EXIT` REPLACES the first rather than adding to it,
# so registering this separately would have silently stopped cleaning up $reaper_tmp above.
trap 'rm -rf "${reaper_tmp:-}" "$args_dir"' EXIT
printf '%s' "$issues" >"$args_dir/open_issues.json"
printf '%s' "$closed" >"$args_dir/closed_issues.json"
printf '%s' "$vars" >"$args_dir/variables.json"
printf '%s' "$secrets" >"$args_dir/secrets.json"
printf '%s' "$observations" >"$args_dir/gate_observations.json"
printf '%s' "$reaper_observations" >"$args_dir/orphan_reaper_observations.json"

jq -n \
	--arg derived_at "$derived_at" \
	--arg previous_derived_at "$prev_derived" \
	--arg repo "$REPO" \
	--slurpfile open_issues "$args_dir/open_issues.json" \
	--slurpfile closed_issues "$args_dir/closed_issues.json" \
	--argjson open_issues_truncated "$open_truncated" \
	--argjson closed_issues_truncated "$closed_truncated" \
	--slurpfile variables "$args_dir/variables.json" \
	--slurpfile secrets "$args_dir/secrets.json" \
	--slurpfile gate_observations "$args_dir/gate_observations.json" \
	--slurpfile orphan_reaper_observations "$args_dir/orphan_reaper_observations.json" \
