...
# ── THE COMPOSITION READS ITS BIG INPUTS FROM FILES, NOT FROM argv. ──
#
# This was `--argjson open_issues "$issues" --argjson closed_issues "$closed" …`, and it died:
#
#     scripts/programme-fetch.sh: line 368: /usr/bin/jq: Argument list too long
#
# Linux caps a SINGLE argv entry at MAX_ARG_STRLEN — 131072 bytes — independently of the much
# larger ARG_MAX for the whole vector, so headroom in the total does not save one oversized
# argument. Measured on 2026-09-08: `$closed` is 194183 bytes over 804 issues, so that one argument
# is half again the cap on its own. The list only grows, so it does not come back under.
#
# It reproduces on a Mac only at a different threshold, which is worth knowing before anyone
# "cannot repro": macOS has no per-argument cap, so the same command survives the real payload here
# and fails at the 1 MiB total instead. Same exit code (126), different limit — do not read a local
# pass as evidence.
#
# The failure mode is what makes this worth a comment rather than a one-line change. The refresher
# ran nightly and failed nightly for a week, with no signal anywhere near it; what people SAW was
# `Authz / open-core guards` going red on every unrelated PR in the repository, blaming a stale
# snapshot. `programme.yml`'s own header predicted exactly that ("a silently dead refresher … reds
# `pnpm check:programme` for everyone about a week later"), and it was right.
#
# `--slurpfile` reads a file as a STREAM of JSON values into an array, so a file holding one
# document arrives as `$name[0]`. That indirection is the reason `arg_file` exists rather than a
# bare `printf`: --slurpfile over an EMPTY file yields `[]`, whose `[0]` is `null`, which would
# write `open_issues: null` into the snapshot and reach the rollup as a shape it does not accept.
# An unset variable takes its stated default instead, so an empty file is not reachable.
arg_dir="$(mktemp -d)"
trap 'rm -rf "$reaper_tmp" "$arg_dir"' EXIT

# arg_file writes one jq input and echoes its path. $2 is the value; $3 is what to write when the
# value is empty, which is a defaulting decision and so is spelled at each call site.
arg_file() {
	local name="$1" value="$2" fallback="$3"
	if [ -z "$value" ]; then value="$fallback"; fi
	printf '%s' "$value" >"$arg_dir/$name.json"
	printf '%s' "$arg_dir/$name.json"
}
...
jq -n \
	...
	--arg repo "$REPO" \
	--slurpfile open_issues "$(arg_file open_issues "$issues" '[]')" \
	--slurpfile closed_issues "$(arg_file closed_issues "$closed" '[]')" \
	--argjson open_issues_truncated "$open_truncated" \
	--argjson closed_issues_truncated "$closed_truncated" \
	--slurpfile variables "$(arg_file variables "$vars" '[]')" \
	--slurpfile secrets "$(arg_file secrets "$secrets" '[]')" \
	--slurpfile gate_observations "$(arg_file gate_observations "$observations" '[]')" \
	--slurpfile orphan_reaper_observations "$(arg_file orphan_reaper "$reaper_observations" '[]')" \
    ...
    gate_observations: $gate_observations[0],
    orphan_reaper_observations: $orphan_reaper_observations[0]
  }' >"$OUT"
