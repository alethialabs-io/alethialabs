/* Vertex Web UI kit — marketing landing recreation. */
    ...
    <header style={{ position: "sticky", top: 0, zIndex: 40, borderBottom: "1px solid var(--border)", background: "color-mix(in oklch, var(--canvas) 82%, transparent)", backdropFilter: "blur(10px)" }}>
      <Wrap style={{ height: 60, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9, color: "var(--text-primary)" }}>
          <WMark size={24} /><span style={{ fontFamily: "var(--font-display)", fontSize: 18, fontWeight: 600, letterSpacing: "-0.01em" }}>Vertex</span>
...
function Features() {
  ...
  return (
    <section style={{ padding: "72px 0", borderTop: "1px solid var(--border)" }}>
      <Wrap>
        <p style={{ ...ebw, marginBottom: 14 }}>Why Vertex</p>
...
/* ---- Ecosystem / modules ---- */
function Ecosystem() {
  const mods = [["Vertex", "Web control plane", "Visual infrastructure configuration, job dashboard, real-time logs, cost estimation."], ["vertex-cli", "CLI + worker", "An interactive terminal wizard for plan, deploy, and teardown across clouds."]];
  return (
            ...
            <WCard key={t} interactive>
              <WCB style={{ display: "flex", gap: 14 }}>
                <span style={{ display: "grid", placeItems: "center", width: 40, height: 40, borderRadius: "var(--radius-md)", background: "var(--surface-muted)", color: "var(--text-primary)", flexShrink: 0 }}>
                  {t === "Vertex" ? <WMark size={20} /> : <svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"><path d="m4 17 6-6-6-6M12 19h8" /></svg>}
                </span>
                <div>
    ...
    <footer style={{ borderTop: "1px solid var(--border)", padding: "56px 0 32px" }}>
      <Wrap>
        <div style={{ display: "grid", gridTemplateColumns: "1.4fr repeat(4,1fr)", gap: 28, marginBottom: 44 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 9, color: "var(--text-primary)", marginBottom: 14 }}><WMark size={22} /><span style={{ fontFamily: "var(--font-display)", fontSize: 17, fontWeight: 600 }}>Vertex</span></div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 22 }}>
          <p style={{ ...ebw, fontSize: 10, margin: 0 }}>© 2026 Alethia · Vertex Systems</p>
